...
from utils.slam.utils import selectPointsFromVoxel
...
def mapObjects(cam_intrinsic, extrinsic_inv,image_height,image_width,objects):
    ...
    for j in range(len(objects)):
        points_homogeneous = np.hstack((np.array(objects[j]['pcd'].points), np.ones((np.array(objects[j]['pcd'].points).shape[0], 1))))
        points_homogeneous = (extrinsic_inv @ points_homogeneous.T).T[:, :3]
        # points_homogeneous[:, [1, 2]] = points_homogeneous[:, [2, 1]]
        points_camera = cam_intrinsic @ points_homogeneous.T
        points_camera = points_camera.T
        z=points_camera[:,-1]
        u=(points_camera[:,0]/z).astype(np.int32)
        v=(points_camera[:,1]/z).astype(np.int32)
        # v = (image_height - points_camera[:,1] / z).astype(np.int32)
        # v = image_height - v
        index = np.where((u>=0)*(u<image_width)*(v>=0)*(v<image_height)*(z>=0))[0]
        index_buffer = np.where((depth_buffer[v[index],u[index]]==0) | (depth_buffer[v[index],u[index]]>z[index]))[0]
...
def generateProjectMatrix(intrinsic,extrinsic):
    ...
    return extrinsic
def generateSoMImage(cfg,image_objects_dict,objects,pkl_pth,label_mode='1', alpha=0.3, anno_mode=['Mask','Mark']):
    for img,idxs in image_objects_dict.items():
        save_dict_name = os.path.join(pkl_pth,"save_dict_"+img.split('/')[-1].split('.')[0]+".pkl")
        # save_dict_name = os.path.join(img[:img.find('image')]+"pkl/","save_dict_"+img[img.find('image')+6:].split('.')[0]+".pkl")
        res = read_pkl(save_dict_name)
        image_rgb, cam_K,pose = res['rgb'].cpu().numpy(),res['cam_K'][:3,:3],res['pose'].cpu().numpy()
