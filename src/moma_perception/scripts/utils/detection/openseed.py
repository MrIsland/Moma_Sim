...
def InitDetection():
    ...
    pretrained_pth = "./pretrained_models/openseed_o365.pt"
...
def convertMask(mask_origin,ids):
    ...
    return masks
def detection(color_img,idx,visual_save_path=None,detection_save_path=None):
    with torch.no_grad():
        image_ori = Image.open(color_img).convert("RGB")
        ...
        demo.save(os.path.join(visual_save_path, str(color_img).split('/')[-1][:-4]+'.png'))
        ### save result
        ...
        if len(labels) > 0 :
            ...
            if len(category_id) > 0 :
                ...
                if detection_save_path is not None:
                    with gzip.open(detection_save_path+str(color_img).split('/')[-1][:-4]+".pkl.gz", "wb") as f:
                        pkl.dump(results, f)
        else :
            results=None
    return results
