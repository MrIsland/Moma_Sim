...
from model.resnet import resnet18, ResNetEncoder

### todo: 增加pointnet和resnet
class PointNavResNetNet(NetworkBuilder.BaseNetwork):
    def __init__(self, params, **kwargs):
        ...
        if self.ve_net is not None:
            num_ve_input = self.ve_in_channels * self.ve_img_h * self.ve_img_w
            state_input_shape = input_shape[0] - 640 - num_ve_input
            mlp_input_shape = self._calc_input_size((state_input_shape + 128 + 512,), self.actor_cnn)
        else:
    ...
    def forward(self, obs_dict):
        ...
        if self.ve_net is not None:
            num_ve_input = self.ve_in_channels * self.ve_img_h * self.ve_img_w
            ve_input = obs[:, -num_ve_input:].reshape(obs.shape[0], self.ve_in_channels, self.ve_img_h, self.ve_img_w)
            ve_feature = self.visual_encoder(ve_input)
            other_obs = obs[:, :-num_ve_input]
        else:
            other_obs = obs
        grasp_feature = self.grasp_pos_encoder(other_obs[:, -640:].reshape(obs.shape[0], -1, 10).permute(0, 2, 1))
        if self.ve_net is not None:
            obs = torch.cat((other_obs[:, :-640], grasp_feature, ve_feature), dim=1)
        else:
