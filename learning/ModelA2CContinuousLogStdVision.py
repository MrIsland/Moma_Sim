...
class ModelA2CContinuousLogStdVision(BaseModel):
    ...
    class Network(BaseModelNetwork):
        def __init__(self, a2c_network, **kwargs):
            kwargs['obs_shape'] = kwargs['obs_shape'][0] - a2c_network.num_ve_input
        ...
        def forward(self, input_dict):
            num_ve_input = self.a2c_network.num_ve_input
            is_train = input_dict.get('is_train', True)
            prev_actions = input_dict.get('prev_actions', None)
            state_grasp_obs = self.norm_obs(input_dict['obs'][:, :-num_ve_input])
            vision_obs = input_dict['obs'][:, -num_ve_input:]
            # pdb.set_trace()
            input_dict['obs'] = torch.cat((state_grasp_obs, vision_obs), dim=1)
