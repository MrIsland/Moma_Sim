...
from utils.common import random_z_rotation_quaternion, mov, load_urdf_with_txt, pose_to_matrix, rotmat_to_euler_xyz_batch,\
    get_quat_180, sample_from_regions_np, opengl_trans_opencv, pre_process_rgb_img, low_pass_filter
...
"""
New urdf:   [装有新的架高架子，有新的lidar, base的部分简化了，将轮子等信息简化。]
...
class MomaMovePickBottleVisionWGraspPosDRTwoCam(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        # self.cfg["env"]["numObservations"] = 29 + 640 + num_visual_observation if self.control_type == "osc" else 26
        # self.cfg["env"]["numObservations"] = 23 + 640
        # self.cfg["env"]["numObservations"] = 15 + 640 + num_visual_observation
        ...
        # self.proj_matrices = []
        self.camera1_actors = []
        self.camera1_rgb_tensor_list = []
        self.camera1_depth_tensor_list = []
        self.camera1_seg_tensor_list = []

        self.camera2_actors = []
        self.camera2_rgb_tensor_list = []
        # self.camera2_depth_tensor_list = []
        # self.camera2_seg_tensor_list = []
        ...
        self.d_gg_pool, self.r_o2g_pool =\
            torch.zeros((self.num_envs, self.max_episode_length)), torch.zeros((self.num_envs, self.max_episode_length))
        self.d_gripper_grasp_pos = torch.zeros((self.num_envs, self.max_episode_length))
        self.reset_idx(torch.arange(self.num_envs, device=self.device))
        self.curr_robot_vel = torch.zeros((self.num_envs, 6))
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        moma_dof_stiffness = to_torch([5000, 5000, 5000, 5000, 5000, 5000, 5000, 5000, 5000, 5000, 5000, 5000], dtype=torch.float32,
                                      device=self.device)
        moma_dof_damping = to_torch([50, 50, 50, 50, 50, 50, 100, 100, 100, 100, 100, 100], dtype=torch.float32,
                                    device=self.device)
        ...
        link_to_camera1_transform = gymapi.Transform()
        link_to_camera1_transform.p = gymapi.Vec3(0.106, 0, 0)
        link_to_camera1_transform.r = gymapi.Quat.from_euler_zyx(0, -np.pi / 2, np.pi)

        self.camera1_asset = gymapi.CameraProperties()
        self.camera1_asset.width = 640
        self.camera1_asset.height = 480
        self.camera1_asset.horizontal_fov = 60.0
        self.camera1_asset.enable_tensors = True
        camera1_u = torch.arange(0, self.camera1_asset.width)
        camera1_v = torch.arange(0, self.camera1_asset.height)
        self.camera1_v2, self.camera1_u2 = torch.meshgrid(camera1_v, camera1_u, indexing='ij')
        self.camera1_u2 = to_torch(self.camera1_u2, device=self.device)
        self.camera1_v2 = to_torch(self.camera1_v2, device=self.device)

        link_to_camera2_transform = gymapi.Transform()
        link_to_camera2_transform.p = gymapi.Vec3(-0.106, 0, 0)
        link_to_camera2_transform.r = gymapi.Quat.from_euler_zyx(0, -np.pi / 2, np.pi)

        self.camera2_asset = gymapi.CameraProperties()
        self.camera2_asset.width = 640
        self.camera2_asset.height = 480
        self.camera2_asset.horizontal_fov = 60.0
        self.camera2_asset.enable_tensors = True
        camera2_u = torch.arange(0, self.camera2_asset.width)
        camera2_v = torch.arange(0, self.camera2_asset.height)
        self.camera2_v2, self.camera2_u2 = torch.meshgrid(camera2_v, camera2_u, indexing='ij')
        self.camera2_u2 = to_torch(self.camera2_u2, device=self.device)
        self.camera2_v2 = to_torch(self.camera2_v2, device=self.device)
        ...
        # todo: 增加墙的时候这部分要修改
        max_agg_bodies = num_moma_bodies + 2 + 4 + 3  # 1 for table, 4 for walls
        max_agg_shapes = num_moma_shapes + 2 + 4 + 3  # 1 for table, 4 for walls
        ...
        for i in range(self.num_envs):
            ...
            camera1_actor = self.gym.create_camera_sensor(env_ptr, self.camera1_asset)
            self.gym.attach_camera_to_body(camera1_actor, env_ptr, link6_id, link_to_camera1_transform, gymapi.FOLLOW_TRANSFORM)
            self._load_camera1(env_ptr, camera1_actor)
            self.camera1_actors.append(camera1_actor)

            # load_camera2, base
            arm_base_link_id = self.gym.find_actor_rigid_body_handle(env_ptr, moma_actor, 'arm_base_link')
            camera2_actor = self.gym.create_camera_sensor(env_ptr, self.camera2_asset)
            self.gym.attach_camera_to_body(camera2_actor, env_ptr, arm_base_link_id, link_to_camera2_transform, gymapi.FOLLOW_TRANSFORM)
            self._load_camera2(env_ptr, camera2_actor)
            self.camera2_actors.append(camera2_actor)
        ...
        self.filtered_actions = torch.zeros(self.num_envs, 2, device=self.device)
        # Setup data
        self.init_data()

    def _load_camera1(self, env_ptr, camera_actor):
        ...
        self.camera1_rgb_tensor_list.append(rgb_tensor)
        self.camera1_depth_tensor_list.append(depth_tensor)
        self.camera1_seg_tensor_list.append(seg_tensor)

    def _load_camera2(self, env_ptr, camera_actor):
        raw_rgb_tensor = self.gym.get_camera_image_gpu_tensor(self.sim, env_ptr, camera_actor, gymapi.IMAGE_COLOR)
        rgb_tensor = gymtorch.wrap_tensor(raw_rgb_tensor)
        # raw_depth_tensor = self.gym.get_camera_image_gpu_tensor(self.sim, env_ptr, camera_actor, gymapi.IMAGE_DEPTH)
        # depth_tensor = gymtorch.wrap_tensor(raw_depth_tensor)
        # raw_seg_tensor = self.gym.get_camera_image_gpu_tensor(self.sim, env_ptr, camera_actor,
        #                                                       gymapi.IMAGE_SEGMENTATION)
        # seg_tensor = gymtorch.wrap_tensor(raw_seg_tensor)
        self.camera2_rgb_tensor_list.append(rgb_tensor)
        # self.camera2_depth_tensor_list.append(depth_tensor)
        # self.camera2_seg_tensor_list.append(seg_tensor)
    ...
    def init_data(self):
        ...
        self.handles = {
            ...
            'link6': self.gym.find_actor_rigid_body_handle(env_ptr, moma_handle, 'link6'),
    ...
    def _update_states(self):
        ...
        curr_rgb_img1_tensor_batch = self.curr_camera1_rgb_tensor_batch[..., :3].float()
        curr_rgb_img1_tensor_batch = pre_process_rgb_img(curr_rgb_img1_tensor_batch,
                                                        img_h=self.cfg_visual_encoder['img_h'],
                                                        img_w=self.cfg_visual_encoder['img_w'])
        ve_feature1 = self.depth_anything(curr_rgb_img1_tensor_batch)
        # pdb.set_trace()
        self.states.update({
            # Robot
            "q": self._q[:, :],
            ...
            'curr_rgb_img1_tensor_batch': curr_rgb_img1_tensor_batch.reshape(self.num_envs, -1),
            've_feature1': ve_feature1.reshape(self.num_envs, -1),
            ...
            "flag_gripper_grasp_pos": (
                        self.d_gripper_grasp_pos[torch.arange(self.num_envs), self.progress_buf] < 0.1).view(-1, 1)
        })
    ...
    def compute_d_gg(self, grasp_poses_factory):
        curr_grasp_pos_tensor = to_torch(np.stack(grasp_poses_factory, axis=0))
        # Tensor(num_envs, 64, 8): [pos, quat, score]
        ...
        d_gg = math.exp(-1) * min_pos_quat_dist
        self.d_gg_pool, self.d_gripper_grasp_pos = to_torch(self.d_gg_pool), to_torch(self.d_gripper_grasp_pos)
        self.d_gg_pool[torch.arange(self.num_envs), self.progress_buf] = d_gg

        self.d_gripper_grasp_pos[torch.arange(self.num_envs), self.progress_buf] = min_pos_quat_dist

    def compute_o2g_reward(self, grasp_poses_factory):
        ...
        #         self.r_o2g_pool[i].append(0)
        #     else:
        #         t_1 = self.progress_buf[i] - 1
        #         t = self.progress_buf[i]
        #         # sigma = 1 / (1 + math.exp(0.5 - (self.frame_idx / self.max_episode_length)))
        #         # r_go = self.score_pool[t] - self.score_pool[t_1]
        #         r_gg = self.d_gg_pool[i][t_1] - self.d_gg_pool[i][t]
        #         r_o2g = 0.5 * r_gg
        #         self.r_o2g_pool[i].append(r_o2g)
        self.compute_d_gg(grasp_poses_factory)
    ...
    def compute_good_grasp_pos(self, radius=0.1, num_grasps=20):
        bottle_pos = self._bottleA_state[:, :3]
        bottle_quat = self._bottleA_state[:, 3:7]
        grasp_poses_factory = []
        for pos, quat in zip(bottle_pos, bottle_quat):
            z_axis = quat_apply(quat, to_torch([0, 0, 1]))
            z_axis = z_axis.cpu().numpy()
            if np.abs(np.dot(z_axis, [0, 0, 1])) < 0.99:
                ortho_vec = np.array([0, 0, 1])
            else:
                ortho_vec = np.array([0, 1, 0])
            x_axis = np.cross(z_axis, ortho_vec)
            x_axis /= np.linalg.norm(x_axis)
            y_axis = np.cross(z_axis, x_axis)

            grasp_poses = self.generate_grasp_pos(pos.cpu().numpy(), x_axis, y_axis, z_axis, radius=radius ,num_grasps=num_grasps) # (num_grasps, 7)
            grasp_poses_factory.append(grasp_poses)
        return grasp_poses_factory
    ...
    def generate_grasp_pos(self, pos, x_axis, y_axis, z_axis, radius, num_grasps):
        # Generate all thetas
        thetas = np.linspace(0, 2 * np.pi, num_grasps, endpoint=False)
        cos_theta = np.cos(thetas)
        sin_theta = np.sin(thetas)

        # (num_grasps, 3)
        offsets = radius * (np.outer(cos_theta, x_axis) + np.outer(sin_theta, y_axis))
        grasp_positions = pos[None, :] + offsets

        # Compute z_g (pointing toward center)
        x_g = -offsets / np.linalg.norm(offsets, axis=1, keepdims=True)
        y_g = np.tile(z_axis, (num_grasps, 1))  # (num_grasps, 3)
        z_g = np.cross(x_g, y_g)

        # Re-orthonormalize in case of numerical issues
        z_g /= np.linalg.norm(z_g, axis=1, keepdims=True)
        y_g = np.cross(z_g, x_g)

        # Build rotation matrices
        R_grasp = np.stack([x_g, y_g, z_g], axis=2)  # shape: (num_grasps, 3, 3)

        # Convert to quaternion
        grasp_quats = R.from_matrix(R_grasp).as_quat()  # shape: (num_grasps, 4)

        # Combine
        grasp_poses = np.concatenate([grasp_positions, grasp_quats], axis=1)
    ...
    def _refresh(self):
        ...
        self.curr_camera1_rgb_tensor_batch, self.curr_camera1_depth_tensor_batch, self.curr_frame_pcd, \
            self.curr_frame_valid, self.camera1_view_camera_matric_inv_batch, self.delta_pos_env \
            = self.acquire_twist_camera_image(self.frame_idx)
    ...
    def collision_detecton_with_wall(self, collision_threshold = 0.1):
        wall_ids = [self.handles[wall_name] for wall_name in self._contact_wall]
    ...
    def compute_observations(self):
        self._refresh()
        # obs = ["bottleA_pos", "bottleA_quat", "eef_pos", "eef_quat", "robot_vel",
    ...
    def _reset_init_robot_state(self, env_ids):
        ...
        if self.moma_position_noise >= 0:
            ...
            init_state_pos[:, :2] = to_torch(rand_xy) + self.moma_default_state_pos[:2]
            # moma_start_pose.p = gymapi.Vec3(-0.7 + rand_xy[0], -0.7 + rand_xy[1], 0.0)
        # init_state_pos[:, 2] = 0.1
        if self.moma_rotation_noise > 0:
            ...
            q_noise = axisangle2quat(to_torch(rand_rot))  # (N, 4) xyzw
    ...
    def _reset_init_obstacle_state(self, env_ids):
        ...
        sampled_obstacle_state[:, :, 0] = to_torch(xs)
        sampled_obstacle_state[:, :, 1] = to_torch(ys)
    ...
    def reset_idx(self, env_ids):
        ...
        self._reset_init_obstacle_state(env_ids=env_ids)        # self._i = True
        ...
        # for grasp_pos_factory in self.grasp_pose_factories:
        #     grasp_pos_factory.reset_gltree()
    ...
    def move_gripper(self, u_gripper):
        ...
        # print('u_gripper:    {};   tmp_u_finger:     {}'.format(u_gripper, tmp_u_finger))
    ...
    def move_base_position(self, u_wheel):
        ...
        ans_base_position[:, 3:7] = to_torch(new_quats)
        w_world = to_torch(np.zeros((wz.shape[0], 3)))
        w_world[:, 2] = wz
    ...
    def save_camera1_image(self, frame_idx):
        for i in range(self.num_envs):
            rgb_tensor = self.camera1_rgb_tensor_list[i]
            depth_tensor = self.camera1_depth_tensor_list[i]
            seg_tensor = self.camera1_seg_tensor_list[i]
            os.makedirs(f'./test_vis/twist_camera/{i}', exist_ok=True)
            if rgb_tensor is not None:
                color_np = rgb_tensor.cpu().numpy()
                print('color_np.shape:  {}'.format(color_np.shape))
                color_np = np.reshape(color_np, (self.camera1_asset.height, self.camera1_asset.width, 4))[:, :, :3]
                cv2.imwrite(f"./test_vis/twist_camera/{i}/rgb_{self.frame_idx:03d}.png", cv2.cvtColor(color_np, cv2.COLOR_RGB2BGR))
            if depth_tensor is not None:
                depth_np = depth_tensor.cpu().numpy()
                depth_np = np.reshape(depth_np, (self.camera1_asset.height, self.camera1_asset.width))
                depth_np = np.nan_to_num(-depth_np, posinf=1e10, neginf=0)
                depth_np = np.clip(depth_np, 0, 5)
                cv2.imwrite(f"./test_vis/twist_camera/{i}/depth_{self.frame_idx:03d}.png", (depth_np * 50).astype(np.uint8))

            if seg_tensor is not None:
                seg_np = seg_tensor.cpu().numpy()
                unique_ids = np.unique(seg_np)
                print('unique_ids:   {}'.format(unique_ids))
                seg_np = np.reshape(seg_np, (self.camera1_asset.height, self.camera1_asset.width))
                cv2.imwrite(f"./test_vis/twist_camera/{i}/seg_{self.frame_idx:03d}.png", seg_np.astype(np.uint8) * 80)

    def save_camera2_image(self, frame_idx):
        for i in range(self.num_envs):
            rgb_tensor = self.camera2_rgb_tensor_list[i]
            os.makedirs(f'./test_vis/base_camera/{i}', exist_ok=True)
            if rgb_tensor is not None:
                color_np = rgb_tensor.cpu().numpy()
                color_np = np.reshape(color_np, (self.camera2_asset.height, self.camera2_asset.width, 4))[:, :, :3]
                cv2.imwrite(f"./test_vis/base_camera/{i}/rgb_{self.frame_idx:03}.png", cv2.cvtColor(color_np, cv2.COLOR_RGB2BGR))

    def acquire_twist_camera_image(self, frame_idx):
        self.gym.render_all_camera_sensors(self.sim)
        self.gym.start_access_image_tensors(self.sim)
        vinv_matrices, proj_matrices = [], []
        delta_pos_env = []
        for i in range(self.num_envs):
            env_ptr, camera1_actor = self.envs[i], self.camera1_actors[i]
            vinv_mat = torch.inverse(
                (to_torch(self.gym.get_camera_view_matrix(self.sim, env_ptr, camera1_actor), device=self.device)))
            proj_mat = to_torch(self.gym.get_camera_proj_matrix(self.sim, env_ptr, camera1_actor), device=self.device)
        ...
        camera1_rgb_tensor_batch = torch.stack(self.camera1_rgb_tensor_list)
        camera1_depth_tensor_batch = torch.stack(self.camera1_depth_tensor_list)
        camera1_seg_tensor_batch = torch.stack(self.camera1_seg_tensor_list)
        camera1_view_camera_matric_inv_batch = torch.stack(vinv_matrices)
        camera1_proj_matrix_batch = torch.stack(proj_matrices)
        height, width = camera1_depth_tensor_batch.shape[1], camera1_depth_tensor_batch.shape[2]
        ...
        frame_pcd, frame_valid = None, None
        # self.save_camera1_image(frame_idx)
        self.gym.end_access_image_tensors(self.sim)
        return camera1_rgb_tensor_batch, camera1_depth_tensor_batch,\
               frame_pcd, frame_valid, camera1_view_camera_matric_inv_batch, delta_pos_env

    def acquire_base_camera_image():
        #### 写到这里了，base的camera往后走
        print('acquire_base_camera_image')
    ...
    def debug_vis_draw_grasp_pos(self):
        if self.viewer and self.debug_vis_grasp_pos:
            ...
            for i in range(self.num_envs):
                list_scene_node = self.grasp_poses_factory[i]
