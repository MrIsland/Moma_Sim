...
class MomaPickAndPlace(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        # obs include: cubeA_pose (7) + eef_pose (6) + q_gripper (6)  20
        # obs include: cubeA_pose (7) + eef_pose (6) + q_gripper (6) + target_cubeA_pose(7) 27
        self.cfg["env"]["numObservations"] = 27 if self.control_type == "osc" else 26
        ...
        self.moma_default_dof_pos = to_torch(
            [0.0, 0.0, 0.65, 0.0, 1.36, 1.57, 0.5, -0.5, 0.5, 0.5, 0.5, -0.5], device=self.device
        )
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        moma_start_pose.p = gymapi.Vec3(-0.7, 0.0, 0.0)
        moma_start_pose.r = gymapi.Quat(0.0, 0.0, 0.0, 1.0)
        ...
        max_agg_bodies = num_moma_bodies + 2  # 1 for table, cubeA
        max_agg_shapes = num_moma_shapes + 2  # 1 for table, cubeA
    ...
    def init_data(self):
        ...
        self.handles = {
            # moma
            # "hand": self.gym.find_actor_rigid_body_handle(env_ptr, moma_handle, "panda_hand"),
        ...
        _actor_root_state_tensor = self.gym.acquire_actor_root_state_tensor(
            self.sim)  # gymTensor(6, 13) 获得所有actor的根状态张量(13)
        ...
        _rigid_body_state_tensor = self.gym.acquire_rigid_body_state_tensor(
            self.sim)  # gymTensor(60, 13) 获取所有刚体的状态张量(13)
        self._root_state = gymtorch.wrap_tensor(_actor_root_state_tensor).view(self.num_envs, -1,
                                                                               13)  # Tensor(num_env, num_actor, 13)
        self._dof_state = gymtorch.wrap_tensor(_dof_state_tensor).view(self.num_envs, -1,
                                                                       2)  # Tensor(num_env, num_joint, 13)
        self._rigid_body_state = gymtorch.wrap_tensor(_rigid_body_state_tensor).view(self.num_envs, -1,
                                                                                     13)  # Tensor(num_env, num_link, 13)
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:] = compute_moma_reward(
            self.reset_buf, self.progress_buf, self.actions, self._init_states, self.states, self.reward_settings,
            self.max_episode_length
        )

    def compute_observations(self):
        self._refresh()
        obs = ["cubeA_quat", "cubeA_pos", "eef_pos", "eef_quat", "target_cubeA_pos", "target_cubeA_quat"]  # 4 + 3 + 3 + 4 + 6 + 7
...
def compute_moma_reward(
        reset_buf, progress_buf, actions, _init_states, states, reward_settings, max_episode_length,
):
    # type: (Tensor, Tensor, Tensor, Dict[str, Tensor], Dict[str, Tensor], Dict[str, float], float) -> Tuple[Tensor, Tensor]
    ...
    # print('_init_states["cubeA_pos"]:  {}, cubeA_pos:  {}, target_cubeA_pos:  {}'.format(_init_states["cubeA_pos"], states["cubeA_pos"], target_cubeA_pos))
    # print('d_cubeA_pos:  {}, dist_target_pos_reward:  {}'.format(d_cubeA_pos, dist_target_pos_reward))
    # print('d_cubeA_pos:   {}'.format(d_cubeA_pos))
    # print('torch.tanh(10.0 * d_cubeA_pos):   {}'.format(torch.tanh(10.0 * d_cubeA_pos)))
    # print('dist_target_pos_reward:   {}'.format(dist_target_pos_reward))
    # Compose rewards
    # rewards = torch.where(
    #     lift_reward,
    ...
    #     + reward_settings["r_target_pos_scale"] * dist_target_pos_reward,
    #     reward_settings["r_dist_scale"] * dist_reward \
    #     + reward_settings["r_lift_scale"] * lift_reward \
    #     + reward_settings["p_gripper_scale"] * penalty_gripper_closing \
    # )
