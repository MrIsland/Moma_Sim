...
from utils.common import random_z_rotation_quaternion, mov
...
class MomaMovePickBottleStateWGraspPos(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.cfg["env"]["numObservations"] = 30 + 640 if self.control_type == "osc" else 30 + 640
        # actions include: delta EEF if OSC (6) or joint torques (7) + bool gripper (1)
        # actions include: base_pose(2) + delta EEF if OSC (6) or joint torques (7) + bool gripper (1) + flag_heuristic
        self.cfg["env"]["numActions"] = 9 if self.control_type == "osc" else 9
        ...
        self.moma_default_state_pos = to_torch(
            [-0.6, -0.7, 0.0, 0.0, 0.0, 0.707, 0.707], device=self.device
        ...
        self.cmd_arm_limit = to_torch([0.05, 0.05, 0.05, 0.1, 0.1, 0.1], device=self.device).unsqueeze(0) if \
            self.control_type == "osc" else self._moma_effort_limits[:6].unsqueeze(0)
        self.cmd_base_limit = to_torch([0.1, 1]) # 0.2 m/s, 2 rad/s
        ...
        self.heuristic_count = torch.ones((self.num_envs, )) * 20
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.num_moma_bodies = self.gym.get_asset_rigid_body_count(moma_asset) + self.gym.get_asset_rigid_body_count(bottle_asset)
        self.num_moma_dofs = self.gym.get_asset_dof_count(moma_asset) + self.gym.get_asset_dof_count(bottle_asset)
        ...
        self._table_surface_pos = np.array([0, 0, table_thickness])
    ...
    def compute_reward(self, actions):
        ...
        # pdb.set_trace()
        heutistic_mask = (actions[:, -1] == -2)
        self.heuristic_count = to_torch(self.heuristic_count)
        self.heuristic_count[heutistic_mask] -= 1
        ...
        # self.reset_buf |= (self.heuristic_count == 0)
        # pdb.set_trace()

    def compute_observations(self):
        self._refresh()
        obs = ["flag_gripper_grasp_pos", "eef_pos", "eef_quat", "bottleA_quat", "bottleA_pos", "robot_vel",
               "bottleA_pos_relative_robot_2d", "flag_collision"]  # 1 + 4 + 3 + 3 + 4 + 6 + 2 + 1
        obs += ["q_gripper"] if self.control_type == "osc" else ["q_gripper"]
    ...
    def _reset_init_bottle_state(self, name, env_ids, check_valid=True):
        ...
        if name.lower() == 'a':
            ...
            cube_heights = self.states["bottleA_height"]
        sampled_cube_state[:, 2] = self._table_surface_pos[2] + 0.14
        sampled_cube_state[:, 6] = 1.0
    ...
    def reset_idx(self, env_ids):
        ...
        self.heuristic_count[env_ids] = 20
    ...
    def move_base(self, u_wheel):
        ...
        vx_local = torch.stack([u_wheel[:, 0], torch.zeros_like(u_wheel[:, 0]), torch.zeros_like(u_wheel[:, 0])], dim=-1)
