...
from utils.common import random_z_rotation_quaternion, mov, pose_to_matrix, pose_to_matrix_batch, rotmat_to_euler_xyz_batch, opengl_trans_opencv
...
class MomaMovePickBottleWGraspPosDR(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        table_thickness = self.cfg['env']['asset'].get('tableHeight', 0.5)  # 0.7
        ...
        num_moma_bodies = self.gym.get_asset_rigid_body_count(self.moma_asset) + self.gym.get_asset_rigid_body_count(bottle_asset)
        num_moma_shapes = self.gym.get_asset_rigid_shape_count(self.moma_asset) + self.gym.get_asset_rigid_shape_count(bottle_asset)
        # todo: 增加墙的时候这部分要修改
    ...
    def collision_detection_with_obs(self, collision_threshold=0.1):
        ...
        obs_contact_force = torch.norm(obs_contact, dim=2).any(dim=1)
    ...
    def pre_physics_step(self, actions):
        ...
        currnet_base_pos_ = self.move_base(u_wheel)
        self._robot_pos_control[:, :] = currnet_base_pos_
        # Deploy actions
