...
class MomaMovePickBottle(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.reward_settings = {
            ...
            'r_grasp_scale': self.cfg["env"]["graspRewardScale"],
        ...
        self.moma_default_state_pos = to_torch(
            ...
            # [-0.55, -0.7, 0.0, 0.0, 0.0, 0.707, 0.707], device=self.device
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_envs):
            ...
            self._bottleA_id = self.gym.create_actor(env_ptr, bottle_asset, bottleA_start_pose, 'bottleA', i, 2, 0)
            # Set colors
    ...
    def _update_states(self):
        ...
        if self.actions is not None:
            if 'flag_gripper_state' in self.states:
                self.states.update({
                    'last_flag_gripper_state': self.states['flag_gripper_state']
                })
            self.states.update({
                "flag_gripper_state": torch.where(self.actions[:, -1] >= 0.0, 1.0, 0.0)
            })
    ...
    def compute_observations(self):
        self._refresh()
        obs = ["bottleA_quat", "bottleA_pos", "eef_pos", "eef_quat",
               "robot_vel", "bottleA_pos_relative_robot_2d", "flag_collision"]  # 4 + 3 + 3 + 4 + 6 + 1
    ...
    def _reset_init_bottle_state(self, name, env_ids, check_valid=True):
        ...
        # sampled_cube_state[:, 2] = self._table_surface_pos[2] + 0.115
        sampled_cube_state[:, 2] = self._table_surface_pos[2] + 0.08
        sampled_cube_state[:, 6] = 1.0
...
def compute_moma_reward(
    ...
    # 1. distance from hand to the bottleA
    ...
    # 2. reward for grasping bottle
    flag_d_ = torch.where(d < 0.015, 1, 0)
    grasping_reward = flag_d_ * (1 - states['flag_gripper_state'])

    # 3. reward for lifting bottleA
    bottleA_height = states["bottleA_pos"][:, 2] - reward_settings["table_height"]
    bottleA_lifted = (bottleA_height - init_bottleA_height) > 0.04
    lift_reward = bottleA_lifted

    # 4. penalty for empty grasping
    d_gripper = torch.norm(states["eef_lf_pos"] - states["eef_rf_pos"], dim=-1)
    penalty_gripper_closing = (d_gripper < 1e-2)

    # 5. penalty for switching gripper states
    penalty_gripper_switching = torch.zeros_like(penalty_gripper_closing)
    if 'last_flag_gripper_state' in states:
        penalty_gripper_switching = (states['last_flag_gripper_state'] != states['flag_gripper_state'])

    # 6. lift the cube to 0.2m more
    target_bottleA_pos = states["target_bottleA_pos"]
    d_bottleA_pos = torch.norm(states["bottleA_pos"] - target_bottleA_pos, dim=-1)
    dist_target_pos_reward = 1 - torch.tanh(10.0 * d_bottleA_pos)
    ...
    rewards = reward_settings["r_dist_scale"] * dist_reward \
              ...
              + reward_settings["r_grasp_scale"] * grasping_reward
              # + reward_settings["p_gripper_scale"] * penalty_gripper_switching

    if DEBUG_REWARD:
        print('!!! rewards:   {}'.format(rewards))
        print('$$  dist_reward:    {};  lift_reward:    {}; grasping_reward:   {}'.format(
            reward_settings["r_dist_scale"] * dist_reward,
            reward_settings["r_lift_scale"] * lift_reward,
            reward_settings['r_grasp_scale'] * grasping_reward
        ))
        print('&&  penalty_gripper_closing:    {};  flag_collision:    {};  flag_gripper_swiching:    {}'.format(
            reward_settings["p_gripper_scale"] * penalty_gripper_closing,
            reward_settings["p_collision_scale"] * flag_collision,
            reward_settings["p_gripper_scale"] * penalty_gripper_switching
        ))
