...
class MomaMovePickBottleVisionWGraspPosDR(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.cfg["env"]["numVisionObservations"] = int(num_visual_observation)  # int(num_visual_observation)
    ...
    def _update_states(self):
        ...
        # state_camera_depth_tensor_batch = torch.clamp(state_camera_depth_tensor_batch, 0, 5)
    ...
    def save_camera_image(self, frame_idx):
        for i in range(self.num_envs):
            ...
            if depth_tensor is not None:
                ...
                cv2.imwrite(f"./test_vis/{i}/depth_{self.frame_idx:03d}.png", (depth_np * 50).astype(np.uint8))
    ...
    def acquire_camera_image(self, frame_idx):
        ...
        for i in range(self.num_envs):
            ...
            vinv_mat = torch.inverse(
                (to_torch(self.gym.get_camera_view_matrix(self.sim, env_ptr, camera_actor), device=self.device)))
        ...
        return camera_rgb_tensor_batch, camera_depth_tensor_batch,\
               frame_pcd, frame_valid, camera_view_camera_matric_inv_batch, delta_pos_env
...
def compute_moma_reward(
    ...
    flag_success_gripper_grasp = (d_gripper_grasp_pos < 0.15)
