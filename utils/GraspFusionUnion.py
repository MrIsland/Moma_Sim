...
class GraspFusion:
    ...
    def _get_target_pcd(self, frame_pcd, frame_valid, target_label):
        pcd = np.asarray(frame_pcd.points)
        color = np.asarray(frame_pcd.colors)
        mask = (frame_valid[:, :, 1] == target_label)
        pcd_valid = pcd[mask.reshape(-1)]
        color_valid = color[mask.reshape(-1)]

        tmp_pcd = o3d.geometry.PointCloud()
        tmp_pcd.points = o3d.utility.Vector3dVector(pcd_valid)
        tmp_pcd.colors = o3d.utility.Vector3dVector(color_valid)

        tmp_pcd, _ = tmp_pcd.remove_radius_outlier(nb_points=6, radius=0.02)
        pcd_valid = np.asarray(tmp_pcd.points)
        color_valid = np.asarray(tmp_pcd.colors)
        axis = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.05)
        if pcd_valid.shape[0] == 0:
            return None
        return to_torch(np.concatenate([pcd_valid, color_valid], axis=1))
