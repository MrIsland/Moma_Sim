...
class MomaPerception:
    def __init__(self, cfg_path):
        ...
        self.control_policy = ModelInference()
    ...
    def twist_callback(self, msg):      
        with self.message_lock:
            self.twist_msg = msg
            self.twist_cnt += 1
            # print('!!!!  self.twist_cnt:       {}'.format(self.twist_cnt))
    ...
    def gripper_state_callback(self, msg): 
        with self.message_lock:
            ...
            self.gripper_cnt += 1
            print('!!!!  self.gripper_cnt:       {}'.format(self.epos_cnt))
    ...
    def moma_cmd_publish_from_action(self, action):        
        ...
        curr_base = chassis_vel

        msg = moma_cmd()
        msg.header.stamp = rospy.Time.now()
        msg.data = np.array([curr_base[0], curr_base[1], delta_ee_pos[0], delta_ee_pos[1], delta_ee_pos[2], 
                             delta_ee_pos[3], delta_ee_pos[4], delta_ee_pos[5], gripper_state], dtype=float)
        ...
        while not rospy.is_shutdown():
            with self.message_lock:
                ...
                if all([
                    ...
                    action = self.control_policy.get_action(data_package, None, 'state', is_deterministic=True)
