...
class IKArmController:
    ...
    def get_ee_pos_rpy_from_pybullet(self, curr_joint_state=None):
        if curr_joint_state is not None:
            # rospy.loginfo('pybullet - current_joint_state:    {}'.format(curr_joint_state))
    ...
    def compute_ik(self, delta_pos, arm_joint_state, arm_ee_pos):
        ...
        rospy.loginfo('#########################################')
        rospy.loginfo('arm_ee_pos:    {}'.format(arm_ee_pos))
        rospy.loginfo('delta_pos:     {}'.format(delta_pos))
        rospy.loginfo('target_pos:    {};   target_quat:    {}'.format(target_pos, target_quat))
        rospy.loginfo('#########################################')
        ...
        joint_angles = p.calculateInverseKinematics(self.robot_id, 
                                                    ...
                                                    solver = p.IK_DLS)
        # rospy.loginfo('joint_angles:           {}'.format(joint_angles))
        # print('##########################################################')
