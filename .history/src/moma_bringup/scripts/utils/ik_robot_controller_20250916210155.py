...
class IKArmController:
    ...
    def get_ee_pos_rpy_from_pybullet(self, curr_joint_state):
    ...
    def get_ee_pos_quat_from_pybullet(self, curr_joint_state):
        # for i, j_idx in enumerate(self.joint_indices):
            # p.resetJointState(self.robot_id, j_idx, curr_joint_state[i])
        link_state = p.getLinkState(self.robot_id, self.end_effector_link_index)
        current_pos = np.array(link_state[4])
    ...
    def compute_target_pos_mul(self, delta_pos):
        # current_pos,current_orn = self.get_ee_pos_rpy_from_pybullet(curr_joint_state)
        current_pos, current_quat = self.get_ee_pos_quat_from_pybullet(curr_joint_state)
    ...
    def compute_ik(self, delta_pos):
        ...
        target_pos, target_quat = self.compute_target_pos_mul(delta_pos)
        # for i,j_idx in enumerate(self.joint_indices):
