...
class ComputeEposeInLidarFrame:
    ...
    def quat_to_matrix(self, quat):
        print('quat[0]:    {};   len(quat):    {}'.format(quat[0], len(quat)))
        tx, ty, tz, qw, qx, qy, qz = quat[0], quat[1], quat[2], quat[3], quat[4], quat[5], quat[6]
        print('tx: {};ty: {}; tz: {}; qw: {}; qx; {}; qx: {}; qy: {}; qz: {}'.format(tx, ty, tz, qw, qx, qy, qz))
    ...
    def compute_epos_in_world(self, T_epos_in_base, quat_base_in_world):
        print('quat_base_in_world:      {}'.format(quat_base_in_world))
        matrix_base_in_world = self.quat_to_matrix(quat_base_in_world)
        print('matrix_base_in_world:     {}'.format(matrix_base_in_world))
    ...
    def publish_synced_msg(self, event):
        try:
            ...
            if self.ee_pos is not None:
                ...
                print('????????????   T_epos_in_base:  {}'.format(T_epos_in_base))
                print('&&&&&&&&&&&&   self.odometry_data:    {}'.format(self.odometry_data))
