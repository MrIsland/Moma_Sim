...
class MomaController:
    def __init__(self, cfg):
        ...
        rospy.Subscriber("/moma_cmd", moma_cmd, self.task_callback, queue_size=1)
        self.cfg = cfg
    ...
    def task_callback(self, msg):
        if msg.data:
            ...
            self.task_cnt += 1
            print('self.task_cnt:     {}'.format(self.task_cnt))
    ...
    def _set_arm_delta_ee_pos(self, delta_ee_pos):
        ...
        curr_pos,curr_orn = self.ik_arm_controller.get_ee_pos_from_pybullet(curr_joint_state)
        curr_quat = p.getQuaternionFromEuler(curr_orn)
        target_pos,target_quat = self.ik_arm_controller.compute_target_pos_mul(delta_ee_pos, curr_joint_state)
        ...
        loop = 0
        for i in range(step):
            tmp_ee_pos = curr_pos + i * delta_ee_pos_interval
            tmp_quat = curr_quat + i * delta_quat_interval
            if loop != 0:
                curr_joint_state = result_joint_state
            result_joint_state = self.ik_arm_controller.compute_ik_without_mul(tmp_ee_pos,tmp_quat,curr_joint_state)
            result_joint_state = np.rad2deg(result_joint_state)
            loop += 1
    ...
    def _move_arm(self, delta_ee_pos):
        print('###################################')
        print('self.arm_joint_ids:       {}'.format(self.arm_joint_ids))
        arm_link1_id = self._get_part_id('joint1', 'joint')
        arm_link6_id = self._get_part_id('joint6', 'joint')
        arm_link1_id = self.robot_model.jnt_qposadr[arm_link1_id]
        arm_link6_id = self.robot_model.jnt_qposadr[arm_link6_id]
        print('self.arm_link1_id:    {};   self.arm_link6_id:    {}'.format(arm_link1_id, arm_link6_id))

        pos, orn = self.ik_arm_controller.get_ee_pos()
        self.arm_ee_pos = np.concatenate([pos, orn])
        target_joint_state = self.ik_arm_controller.compute_ik(delta_ee_pos, self.robot_data.qpos[self.arm_joint_ids])
    ...
    def step(self, action):
        ...
        print('joint_state:               {}'.format(self.robot_data.qpos[self.arm_joint_ids]))
        ...
        #     arm_joint_state = action_tmp[2:8]
    ...
    def _get_state(self):
        ...
        curr_pos,curr_orn = self.ik_arm_controller.get_ee_pos_from_pybullet(joint_state)
