...
class MomaController:
    ...
    def _move_arm(self, delta_ee_pos):
        ...
        arm_joint_state = self.sync_pub.joint_state_data
        # self.arm_joint_state = self.robot_data.qpos[self.arm_joint_ids]
        # pos, orn = self.ik_arm_controller.get_ee_pos_rpy_from_pybullet(curr_joint_state=self.arm_joint_state)
        # rospy.loginfo('self.arm_joint_state:      {}'.format(arm_joint_state))
        arm_ee_pos = np.concatenate([pos, orn])
        target_joint_state = self.ik_arm_controller.compute_ik(delta_ee_pos, arm_joint_state, arm_ee_pos)
    ...
    def _get_state(self):
        ...
        # curr_pos, curr_orn = self.ik_arm_controller.get_ee_pos_rpy_from_pybullet()
